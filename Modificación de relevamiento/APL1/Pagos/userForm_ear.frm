VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} userForm_ear 
   Caption         =   "Formulario de relevamiento - Embarazos de alto riesgo"
   ClientHeight    =   7365
   ClientLeft      =   45
   ClientTop       =   390
   ClientWidth     =   15285
   OleObjectBlob   =   "userForm_ear.frx":0000
   StartUpPosition =   1  'Centrar en propietario
End
Attribute VB_Name = "userForm_ear"
...
If (userForm_ear_verificacion_blancos = 1) Then
...
If (auxiliarInexistenteEAR = 1) Then
...
auxiliarInexistenteEAR = 0

End If


Call userForm_ear_guardar_datos(filaDobleClickEAR)

MsgBox ("Se han guardado con exito")


If (userForm_ear.dato_validacion.Text = "Labrar acta" Or userForm_ear.dato_validacion.Text = "Labrar acta e indicar fuente de información en observaciones") Then

    Cells(filaDobleClickEAR, columnaDobleClickEAR).Value = "Labrar acta"

ElseIf (userForm_ear_verificacion_blancos <> 1) Then

    Cells(filaDobleClickEAR, columnaDobleClickEAR).Value = "Completo"

Else

    Cells(filaDobleClickEAR, columnaDobleClickEAR).Value = "Incompleto"
...
Private Sub dato_altura_uterina_Change()

'con este if se evita que se ingresen datos que no estan permitidos
If (dato_altura_uterina.Text <> "Dato no obligatorio" And dato_altura_uterina.Text <> "Si" And dato_altura_uterina.Text <> "si" And dato_altura_uterina.Text <> "No" _
And dato_altura_uterina.Text <> "no") Then
...
Private Sub dato_apgar_Change()

'con este if se evita que se ingresen datos que no estan permitidos
If (dato_apgar.Text <> "Dato no obligatorio" And dato_apgar.Text <> "Si" And dato_apgar.Text <> "si" And dato_apgar.Text <> "No" _
And dato_apgar.Text <> "no") Then
...
Private Sub dato_n_control_Change()

'con este if se evita que se ingresen datos que no estan permitidos (solo numericos)
If Not (IsNumeric(dato_n_control.Text) Or dato_n_control.Text = "Dato no obligatorio") Then
...
'con este if se evita que se ingresen datos que no estan permitidos
If (dato_perimetro_cefalico.Text <> "Dato no obligatorio" And dato_perimetro_cefalico.Text <> "Si" And dato_ta.Text <> "si" And dato_perimetro_cefalico.Text <> "No" _
And dato_perimetro_cefalico.Text <> "no") Then
...
Private Sub dato_screening_neonatal_Change()

'con este if se evita que se ingresen datos que no estan permitidos
If (dato_screening_neonatal.Text <> "Dato no obligatorio" And dato_screening_neonatal.Text <> "Si" And dato_screening_neonatal.Text <> "si" And dato_screening_neonatal.Text <> "No" _
And dato_screening_neonatal.Text <> "no") Then

dato_screening_neonatal.Text = ""
...
Private Sub dato_fecha_egreso_Change()

'para que siga escribiendo en una linea de inferior
dato_fecha_egreso.MultiLine = True

End Sub

Private Sub dato_fecha_ingreso_Change()

'para que siga escribiendo en una linea de inferior
dato_fecha_ingreso.MultiLine = True
...
Private Sub dato_motivo_egreso_Change()

'para que siga escribiendo en una linea de inferior
dato_motivo_egreso.MultiLine = True

End Sub

Private Sub dato_reporte_Change()

'para que siga escribiendo en una linea de inferior
dato_reporte.MultiLine = True

End Sub

Private Sub dato_sala_Change()

'para que siga escribiendo en una linea de inferior
dato_sala.MultiLine = True
...
'para que siga escribiendo en una linea de inferior
userForm_ear.dato_observaciones.MultiLine = True


End Sub

Private Sub dato_uti_Change()

'para que siga escribiendo en una linea de inferior
dato_uti.MultiLine = True
...
Or dato_fuente.Text = "Prestación inexistente" Or dato_fuente.Text = "Caso duplicado") Then
    ...
    If dato_fuente.Text <> "No consta fuente de información" And dato_fuente.Text <> "Prestación inexistente" And dato_fuente.Text <> "Caso duplicado" Then
        ...
        If (codigoDobleClickEAR <> "") Then
            ...
            Call userForm_ear_permitir_campos_requeridos(codigoDobleClickEAR)
        ...
        On Error Resume Next
            ...
            concatenacion = TextBox_codigo.Text & dato_fuente.Text & ActiveSheet.Cells(filaDobleClickEAR, columnaDobleClickEAR + 27).Value
...
TextBox_monto.Text = Format(TextBox_monto.Text, "Short date")
...
If (Cells(filaDobleClickEAR, columnaDobleClickEAR + 1) = "") Then
dato_validacion.Text = "Ingresar la fuente de información"
dato_validacion.BackColor = RGB(255, 255, 0)
End If
